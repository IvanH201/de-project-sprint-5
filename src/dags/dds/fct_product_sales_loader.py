            ...
            cur.execute(
                """
                    with ss as (
                        ...
                        select 
                            t.id, 
                            (select id from dds.dm_products p where p.product_id = (t.product_json::JSON->>'id') order by id asc limit 1) product_id,
                            (select id from dds.dm_orders o where o.order_key = t.order_id order by id asc limit 1) order_id,
                        ...
                        and exists (select id from dds.dm_orders o where o.order_key = t.order_id order by id asc limit 1)

...
class SettlementOriginRepository:
    ...
    def list_objects(self, settlement_threshold: int, limit: int) -> List[SettlementObj]:
        with self._db.client().cursor(row_factory=class_row(SettlementObj)) as cur:
            cur.execute(
                """
                select restaurant_id, restaurant_name, settlement_date, count(distinct order_key) orders_count, 
                    max(ts) max_ts, sum(total_sum) orders_total_sum, sum(bonus_payment) orders_bonus_payment_sum, 
                    sum(bonus_grant) orders_bonus_granted_sum, sum(total_sum) *0.25 order_processing_fee, 
                    sum(total_sum) - sum(bonus_payment) - sum(total_sum) *0.25 restaurant_reward_sum
                from (
                    select dt.id, fps.order_id, fps.total_sum, fps.bonus_payment, fps.bonus_grant, dr.restaurant_id,
                        dr.restaurant_name, dt."year", dt."month", dt."date", do2.order_key , dt.ts,
...
class SettlementLoader:
    WF_KEY = "cdm_settlement_workflow"

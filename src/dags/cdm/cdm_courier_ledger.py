                ...
                f"""
                    WITH agr AS (
                                SELECT
                                    ...
                                    SUM(fd.delivery_sum) AS orders_total_sum,
                                    AVG (rate) AS rate_avg,
                                    SUM(fd.delivery_sum) * 0.25 AS order_processing_fee,
                                    SUM(fd.tip_sum) AS courier_tips_sum
                                FROM dds.fct_deliveries fd
                                LEFT JOIN dds.dm_couriers dc ON fd.courier_id = dc.id
                                GROUP BY fd.courier_id, dc.courier_name, settlement_year, settlement_month
                                ),
                        agr2 AS(
                                SELECT
                                    agr.courier_id,
                        ...
                        FROM agr2
                        WHERE (agr2.settlement_year=extract(year from ('{current_month}'::date))) and 
                                (agr2.settlement_month=extract(month from ('{current_month}'::date)))
...
class DmCourierLoader:
    ...
    def __init__(self, pg_origin: PgConnect, pg_dest: PgConnect, log: Logger) -> None:
        ...
        self.current_month=self.prev_month
        self.log.info(f"current_month= {self.current_month} ")
    def load_data(self):
        ...
        with self.pg_dest.connection() as conn:
            ...
            self.log.info(f"Found {len(load_queue)} cdm_courier_ledger to load.")
